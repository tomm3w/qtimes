    ...
    public class EmailHelper
    {
        ...
        public void SendConfirmationEmailMail(UserProfile user)
        {
            try
            {
                ...
                {
                    usrInfo = context.UserInfoes.FirstOrDefault(u => u.UserId == user.UserId);

                    if (usrInfo != null)
                    {
                        MailMessage message = new MailMessage { Subject = "QTimes Signup - Confirmation Mail" };
                        message.To.Add(user.Email);
                        message.IsBodyHtml = true;
                        String confirmationURL = ConfigurationHelper.Instance.ConfirmationURL;
                        message.Body = String.Format("Welcome {0},<br/><br/>Thanks for registering, please click on the link below to activate your account.<br/><br/><a href=\"{1}\">{1}</a><br/><br/>Best Regards,<br/>QTimes Team", user.FullName, confirmationURL + "/" + usrInfo.ConfirmationToken);
                        SendMail(message);
                    }
                }
        ...
        public void ContactSupport(ContactModel model)
        {
            try
            {
                MailMessage message = new MailMessage { Subject = "QTimes Contact" };
        ...
        {
            MailMessage message = new MailMessage { Subject = "QTimes Password - Reset Password Mail" };
            message.To.Add(user.Email);
            message.IsBodyHtml = true;
            String confirmationURL = ConfigurationHelper.Instance.ConfirmationURL;
            message.Body = String.Format("Hello {0},<br/><br/>Your new password is <b>{1}</b><br/><br/>Best Regards,<br/>QTimes Team", user.UserName, pwd);
            SendMail(message);
        }
    }
