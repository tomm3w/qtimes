...
namespace QTimes.api.Queries.Reservation.GetBusinessAccounts
{
    public class GetBusinessAccountsQuery : IQuery<GetBusinessAccountsResponse, GetBusinessAccountsRequest>
    {
        ...
        public GetBusinessAccountsResponse Handle(GetBusinessAccountsRequest request)
        {
            ...
            var uId1 = Guid.Parse(request.UserId.ToString()); // Ensure valid GUID
            var user11 = _userInfoRepository.GetAll().SingleOrDefault(x => x.UserId == uId1);
            var uId = new Guid(request.UserId.ToString());
            var user2 = _userInfoRepository.GetAll();
            var user1 = user2.Where(x => x.UserId == uId).ToList();
            var user = user2.Where(x => x.UserId == uId).FirstOrDefault();
