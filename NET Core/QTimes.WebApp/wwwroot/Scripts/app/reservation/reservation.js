...
function ReservationViewModel() {
    ...
    self.LoadBusinessInfo = function () {
        $.ajax({
            type: 'POST',
            url: webserviceURL + "reservation/getsummary",
            data: { Date: moment().format('YYYY-MM-DD'), BusinessDetailId: self.BusinessId(), CurrentTime: moment().format('HH:mm:ss') },
    ...
    self.Load = function (hideloader) {
        ...
        qs = qs + '&ReservationDate=' + self.ReservationDate() + '&BusinessId=' + self.BusinessId();
        if (self.SearchText())
    ...
    self.LoadTimeSlots = function (editdata) {
        ShowLoader();
        $.post(webserviceURL + "reservation/gettimeslots", { Date: self.SelectedDate(), BusinessId: self.BusinessId() }, function (data) {
            ...
            if (editdata) {
                var chkTime = editdata.TimeFrom() + '-' + editdata.TimeTo();
    ...
    self.AddReservation = function () {
        ...
        if (self.Mode() === 'NEW') {
            $.ajax({
                ...
                data: {
                    Name: self.GuestName(),
                    MobileNo: iti.getNumber(), //self.MobileNumber(),
    ...
    self.Edit = function (data) {
        ...
        iti.setNumber('+' + data.GuestMobileNumber());
    ...
    self.Reply = function () {
        $.ajax({
            ...
            success: function (response) {
                ...
                obj['isReplied'] = ko.observable(false);
    ...
    self.ClearAll = function () {
        ...
        $('.timeslots').find('.input_chk').removeClass('tick');
        $('#add-new-reserve_id .theme-black-form .form_second li').removeClass('active');
    };
...
function Reservation(data) {
    ...
    $res.ReservedTime = ko.computed(function () {
        return moment(data.TimeFrom, "HH:mm:ss").format("hh:mm A") + " - " + moment(data.TimeTo, "HH:mm:ss").format("hh:mm A");
    });
    ...
    $res.IsTimeOut = ko.computed(function () {
        if ((data.TimeIn != null && data.TimeOut != null) || data.IsCancelled == true)
            return true;
        else
            return false;
...
function GuestMessage(data) {
    ...
    $message.isReplied = ko.observable(false);
    if (data.MessageSent) {
        $message.Message = ko.observable(data.MessageSent);
        $message.isReplied(false);
    }
    if (data.MessageReplied) {
        $message.Message = ko.observable(data.MessageReplied);
