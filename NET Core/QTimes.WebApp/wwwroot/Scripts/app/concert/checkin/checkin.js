...
function ReservationViewModel() {
    ...
    self.Load = function () {
        ...
        qs = qs + '&ConcertId=' + concertId;
        if (self.SearchText())
            qs = qs + '&SearchText=' + self.SearchText()

        $.ajax({
            type: 'GET',
            url: webserviceURL + `concerts/${concertId}/events/${eventId}/checkin` + qs,
    ...
    self.AddReservation = function (data) {
        ...
        $.ajax({
            ...
            url: webserviceURL + `concerts/${concertId}/events/${eventId}/checkin`,
            data: {
                ConcertGuestId: data.ConcertGuestId(),
                ConcertEventReservationId: data.ConcertEventReservationId(),
...
function Reservation(data) {
    ...
    $res.PhotoFullPath = ko.computed(function () {
        if (data.FacePhotoImageBase64)
            return 'data:image/jpeg;base64,' + data.FacePhotoImageBase64;
        else
...
function Guest(data) {
    var $guest = this;
    $guest.ConcertEventReservationId = ko.observable(data.ConcertEventReservationId);
    ...
    $guest.IsWSomeone = ko.observable(!data.IsSolo);
